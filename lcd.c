...
void display_bmp(char *path)
{
    ...
    if(err!=0)
   {
	...
	while (1) {
		...
		timenow = localtime(&now); //localtime函数把从time取得的时间now换算成你电脑中的时间(就是你设置的地区) 
		color = RGB565(240, 240, 240);//白色
		printlcd(30, 20, asctime(timenow)); //asctime函数把时间转换成字符，通过printf()函数输出 
		printlcd(1080, 680, "by zhaoyunfei 181180202"); //asctime函数把时间转换成字符，通过printf()函数输出 
